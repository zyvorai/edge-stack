#!/usr/bin/env bash
# Render Edge Stack social cards with Google Chrome + macOS `sips`.
...
SOCIAL_OUT="${1:-$HERE/edge-stack-social-card.jpg}"
SHARE_OUT="${2:-$HERE/edge-stack-share-card.png}"

PNG="$(mktemp "${TMPDIR:-/tmp}/edge-stack-social.XXXXXX.png")"
PNG2="$(mktemp "${TMPDIR:-/tmp}/edge-stack-share.XXXXXX.png")"
trap 'rm -f "$PNG" "$PNG2"' EXIT

"$CHROME" --headless=new --disable-gpu --hide-scrollbars --force-device-scale-factor=1 \
  --window-size=1600,900 --screenshot="$PNG" "file://$HERE/edge-stack-social-card.html" >/dev/null 2>&1
...
"$CHROME" --headless=new --disable-gpu --hide-scrollbars --force-device-scale-factor=1 \
  --window-size=1200,630 --screenshot="$PNG2" "file://$HERE/edge-stack-share-card.html" >/dev/null 2>&1
